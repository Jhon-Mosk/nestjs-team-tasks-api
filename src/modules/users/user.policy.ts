...
const ALLOWED_DELETE_USER_ROLES: Record<UserRole, Set<UserRole>> = {
  [UserRole.OWNER]: new Set([UserRole.MANAGER, UserRole.EMPLOYEE]),
  [UserRole.MANAGER]: new Set([UserRole.EMPLOYEE]),
  [UserRole.EMPLOYEE]: new Set([]),
};
...
export const isAllowedToDeleteUser = (
  actorRole: UserRole,
  userRole: UserRole,
): boolean => {
  return ALLOWED_DELETE_USER_ROLES[actorRole].has(userRole) || false;
};
